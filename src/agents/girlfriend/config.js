...
const { shouldTriggerImageGeneration, isScheduledImageRequest, parseDelayMinutes, getImageGenerationParams } = require('./image-trigger');
...
function createGirlfriendAgent({ agentId, characterId = '', heartbeatInterval = 120000, platform = 'qq' } = {}) {
  ...
  const runTurn = async (userMessage, onLog, msgPlatform, meta = {}) => {
    ...
    if (userMessage && shouldTriggerImageGeneration(userMessage)) {
      onLog('info', `[${agentId}] 检测到即时图片请求，自动生成图片...`);
      try {
        const imageParams = getImageGenerationParams(charId, userMessage);
        onLog('info', `[${agentId}] 图片参数: ${JSON.stringify(imageParams)}`);
        ...
        onLog('info', `[${agentId}] 图片已发送: ${result}`);
      } catch (error) {
