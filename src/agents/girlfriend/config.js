  ...
  const runTurn = async (userMessage, onLog, msgPlatform, meta = {}) => {
    ...
    if (userMessage && shouldTriggerImageGeneration(userMessage)) {
      ...
      try {
        ...
        const recentMessages = memoryStore.getMessages(charId).slice(-6);
        ...
        onLog('info', `[${agentId}] LLM 图片参数: prompt="${imageParams.prompt.substring(0, 60)}..." style=${imageParams.style} ratio=${imageParams.aspectRatio}`);
